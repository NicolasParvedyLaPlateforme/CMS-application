...
export default function ResolveClient() {
  ...
  }, [course, id1, id2, type, urlL1, urlL2]);

  if (!item1 || !item2) {
    return (
      <div className="p-8 text-center text-slate-500">
        Item introuvable ou résolu.{" "}
  ...
  const doDelete = (itemNum: 1 | 2) => {
    ...
    if (lSave) {
      ...
      if (type === "word") {
        ...
        u.phrases = u.phrases.map((p) =>
          p.id === itemToSave.id ? itemToSave : p,
        );
      }
    ...
    if (lessonIdToDelete !== lessonIdToSave) {
      const lDel = course.lessons.find(
        (lesson) => lesson.id === lessonIdToDelete,
      );
      if (lDel) {
        const u = { ...lDel };
        if (type === "word") {
  ...
  const saveChanges = () => {
    ...
    if (l1) {
      const u = { ...l1 };
      if (type === "word")
        u.words = u.words.map((w) => (w.id === id1 ? item1 : w));
    ...
    if (l2 && l1?.id !== l2.id) {
      const u = { ...l2 };
      if (type === "word")
        u.words = u.words.map((w) => (w.id === id2 ? item2 : w));
      ...
      if (type === "word") {
        u.words = u.words.map((w) =>
          w.id === id1 ? item1 : w.id === id2 ? item2 : w,
        );
      } else {
        u.phrases = u.phrases.map((p) =>
          p.id === id1 ? item1 : p.id === id2 ? item2 : p,
        );
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 p-6">
      <div className="flex items-center gap-4 border-b border-slate-200 pb-4">
        <Link
          href="/"
          className="p-2 border border-slate-200 hover:bg-slate-100 rounded-md transition-colors text-slate-500 text-sm font-semibold flex items-center gap-2"
        >
          <ArrowLeft size={16} /> Retour
        </Link>
        <h1 className="text-xl font-bold text-slate-800">
          Résoudre le conflit ({type === "word" ? "Mot" : "Phrase"})
        </h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white border text-left border-red-200 shadow-[0_0_0_1px_rgba(248,113,113,0.3)] rounded-lg p-5 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="font-bold text-slate-800 text-[14px] m-0">
              Item 1{" "}
              <span className="font-normal text-slate-500 text-[12px] ml-2">
                (Leçon: {lesson1Id})
              </span>
            </h2>
            ...
            {confirmDelete === 1 ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => doDelete(1)}
                  className="text-[12px] font-semibold text-white bg-red-500 hover:bg-red-600 px-2 py-1 rounded"
                >
                  Oui
                </button>
                <button
                  onClick={() => setConfirmDelete(null)}
                  className="text-[12px] font-semibold text-slate-500 bg-slate-100 hover:bg-slate-200 px-2 py-1 rounded"
                >
                  Non
                </button>
              </div>
              ...
              <button
                ...
                className="text-[12px] font-semibold text-red-500 hover:text-red-700 flex items-center gap-1 bg-red-50 px-2 py-1 rounded"
              >
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <input
                value={item1.id}
                onChange={(e) => handleUpdate(1, "id", e.target.value)}
            ...
            <div className="space-y-1">
              ...
              <input
                value={item1.fr}
                onChange={(e) => handleUpdate(1, "fr", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                value={item1.en}
                onChange={(e) => handleUpdate(1, "en", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                value={item1.th}
                onChange={(e) => handleUpdate(1, "th", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-[18px] text-blue-500 font-semibold"
              />
            </div>
            <div className="space-y-1">
              <label className="text-[11px] font-semibold text-slate-500">
              ...
              <input
                value={item1.phonetic}
                onChange={(e) => handleUpdate(1, "phonetic", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
          </div>
        ...
        <div className="bg-white border text-left border-red-200 shadow-[0_0_0_1px_rgba(248,113,113,0.3)] rounded-lg p-5 space-y-4">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <input
                value={item2.id}
                onChange={(e) => handleUpdate(2, "id", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                value={item2.fr}
                onChange={(e) => handleUpdate(2, "fr", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                value={item2.en}
                onChange={(e) => handleUpdate(2, "en", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                value={item2.th}
                onChange={(e) => handleUpdate(2, "th", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-[18px] text-blue-500 font-semibold"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                value={item2.phonetic}
                onChange={(e) => handleUpdate(2, "phonetic", e.target.value)}
                className="w-full border border-slate-300 rounded px-2.5 py-1.5 text-sm"
              />
            </div>
          </div>
      ...
      <div className="flex justify-end gap-4 mt-6">
        <button
          onClick={saveChanges}
          className="bg-slate-900 text-white font-semibold flex items-center justify-center px-6 py-2 rounded-md hover:bg-slate-800 transition-colors"
        >
          Sauvegarder
        </button>
      </div>
